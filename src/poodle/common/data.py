...
from .util import display_percent
...
@dataclass
class RunResult:
    """Test Run Result for a source folder."""

    result: str
    description: Any = None

    RESULT_PASSED = "All tests passed"
    RESULT_FAILED = "Some tests failed"
    RESULT_TIMEOUT = "Testing exceeded timeout limit"
    RESULT_ERROR = "An error prevented testing completion"
    RESULT_OTHER = "Other, see description"
...
class TestingSummary(PoodleSerialize):
    ...
    def coverage_display(self) -> str:
        ...
        return display_percent(self.success_rate)
    ...
    def from_dict(d: dict[str, Any]) -> dict[str, Any]:
        """Correct fields in Dictionary for JSON deserialization."""
        d.pop("success_rate", None)  # nomut: Keyword
        d.pop("coverage_display", None)  # nomut: Keyword
        return d
