...
def html_reporter(config: PoodleConfig, echo: Callable, testing_results: TestingResults, *_, **__) -> None:
    ...
    common_vars = {
        "project": {
            "name": html_options.get("project_name", None),
            "version": html_options.get("project_version", None),
        },
    ...
    detail_template = env.get_template("html-report-detail.html.jinja")
    detail_page = detail_template.render(modules=modules, **common_vars)
    (report_folder / "detail.html").write_text(detail_page.strip())
...
def module_data(testing_results: TestingResults, html_options: dict) -> dict[Path, dict[str, Any]]:
    ...
    module_dict: dict[Path, dict] = {m: dict() for m in modules}  # noqa: C408 - Unnecessary `dict` call
    include_found = html_options.get("include_found_details", False)
    ...
    for idx, module in enumerate(modules, start=1):
        ...
        module_add_trials_to_lines(module_dict[module]["trials"], module_dict[module]["lines"], include_found)
