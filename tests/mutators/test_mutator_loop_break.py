...
from poodle.mutators import mutator_loop_break


@pytest.fixture(autouse=True)
def _setup():
    importlib.reload(mutator_loop_break)
    yield
    importlib.reload(mutator_loop_break)
...
class TestRegisterPlugins:
    ...
    def test_register_plugins(self):
        ...
        with mock.patch("poodle.mutators.mutator_loop_break.LoopBreakMutator") as mock_bool_op_mutator:
            mutator_loop_break.register_plugins(plugin_manager)
        plugin_manager.register.assert_called_once_with(mock_bool_op_mutator.return_value, "LoopBreakMutator")


class TestLoopBreakMutator:
    def test_vars(self):
        assert mutator_loop_break.LoopBreakMutator.mutator_name == "LoopBreak"

    def test_create_mutations(self):
        mutator = mutator_loop_break.LoopBreakMutator()
    ...
    def test_create_mutations_disabled(self):
        ...
        parsed_ast = ast.parse("continue")
        config = PoodleConfigData()
        config.skip_mutators = ["LoopBreak".lower()]
